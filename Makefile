gdeploy:
	...
	gcloud run deploy mgdev-api --image gcr.io/michaelgreendev/mgdev-api --platform managed --region us-west1 --project michaelgreendev
	...
	gcloud run deploy mgdev-server --image gcr.io/michaelgreendev/mgdev-server --platform managed --region us-west1 --project michaelgreendev
	docker build -f _docker/Dockerfile.blog -t gcr.io/michaelgreendev/mgdev-blog .
	docker push gcr.io/michaelgreendev/mgdev-blog
	gcloud run deploy mgdev-blog --image gcr.io/michaelgreendev/mgdev-blog --platform managed --region us-west1 --project michaelgreendev
